"""Extract the actual release ZIP into a validated temporary root and verify it."""
import hashlib
import json
import os
import subprocess
...
with tempfile.TemporaryDirectory(prefix='l13check-') as folder:
    ...
    with log_path.open('x', encoding='utf-8') as log:
        run = subprocess.run([sys.executable, '-B', 'verify_release.py', '--mode', 'functional', '--out', str(out)],
    ...
    assert report['status'] == 'passed' and report['manifest_files'] == record['manifest_files']
    report.update({'archive': record, 'archive_actually_extracted': True, 'crc_verified': True,
                   'platform_scope': 'Same Windows host, fresh temporary directories; not Linux.',
                   'v012_tests_run_before_packaging': 97,
                   'v012_test_breakdown': {'own': 40, 'inherited_v011': 57},
                   'older_full_evaluations_repeated_this_turn': False,
                   'full_v013_evaluations_before_packaging': 2})
    with (outputs / (name + '-VERIFICATION.json')).open('x', encoding='utf-8') as f:
