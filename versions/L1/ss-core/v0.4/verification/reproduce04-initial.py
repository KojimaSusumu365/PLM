...
def cold(source):
    with tempfile.TemporaryDirectory(prefix='plm-working04-cold-') as d:
        ...
        process=subprocess.run([sys.executable,'-I','-B',str(dest/'entry.py')],cwd=dest,env={**os.environ,'PYTHONIOENCODING':'utf-8'},capture_output=True,text=True,encoding='utf-8',check=True)
