...
BASE=Path(__file__).resolve().parents[1];old=BASE/'outputs/PLM-L1-SS-doc-v0.5';new=BASE/'outputs/PLM-L1-SS-doc-v0.6'
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
assert not new.exists();new.mkdir()
prior={p.relative_to(old).as_posix():sha(p) for p in old.rglob('*') if p.is_file()}
for name in ('ss_partial','ss_document','plm_l1_v09','ss_retention','ss_revision','ss_reconfirm','model','data','tests'):
    shutil.copytree(old/name,new/name)
for name in ('evaluation','verification','examples'):(new/name).mkdir()
for name in ('__init__.py','integrity.py','cases.py','oracle.py','event_oracle.py','revision_cases.py','revision_experiment.py','reconfirm_cases.py','reconfirm_experiment.py'):
