...
def snapshot():
    ...
    for name in ("plm_s1","tests","evaluation","vendor","examples"):
...
def main():
    ...
    if args.create:
        ...
        with MANIFEST.open("x",encoding="utf-8") as f:
            json.dump({"version":"PLM-S1 v0.1","frozen_at_utc":datetime.now(timezone.utc).isoformat(),"files":snapshot()},f,indent=2)
            f.write("\n")
    result=verify()
