...
def fit(train,validation,*,representation='additive',selector='off',backend='ss',dimension=2048,seed='development-0',budget='per_candidate',enabled=True):
    ...
    unknown_validation=[not all(v in vocabulary[f] for f,v in r['context'].items()) for r in validation]
    ...
    for mask in masks:
        ...
        for i,unknown in enumerate(unknown_validation):
            if unknown:predicted[n+i]=None
...
def calibrate(model,rows,risk_target=.1):
    ...
    predictions=[]
    for r in rows:
        try:predictions.append(model.predict(r['context'],threshold=0.))
        except ValueError as e:
            if str(e)!='unknown_field_value':raise
            predictions.append({'value':None,'confidence':0.,'reason':'unknown_field_value'})
