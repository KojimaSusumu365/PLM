"""Seal the active teacher release, then extract and verify the actual archive."""
import json
import os
import subprocess
import sys
...
name='PLM-L1-SS-active-v0.1'
root=workspace/'outputs'/name
sys.path.insert(0,str(root))
from evaluation.integrity import sha,verify,write

frozen=verify()
for file in ('README.md','REPORT.md','verification/isolation/VERIFICATION.json','verification/previous/VERIFICATION.json',
             'verification/SUMMARY.json','verification/BENCHMARK.json','verification/REPEATABILITY.json','verification/PRESERVATION.json'):
...
assert repeat['all_equal'] and len(repeat['files'])==176
...
with tempfile.TemporaryDirectory(prefix='aszip-') as folder:
    ...
    with logpath.open('x',encoding='utf-8') as log:
        process=subprocess.run([sys.executable,'-B','verify_release.py','--out',str(base/'verified')],cwd=base/name,env=env,stdout=log,stderr=subprocess.STDOUT,timeout=600)
    assert process.returncode==0,logpath.read_text(encoding='utf-8')[-2500:]
    ...
    verified.update({'archive':record,'archive_actually_extracted':True,'crc_verified':True,'full_evaluations_before_packaging':2,
                     'repeat_equal_files':176,'preserved_previous_files':len(baseline['previous_files'])})
