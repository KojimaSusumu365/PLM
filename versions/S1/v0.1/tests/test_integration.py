...
from plm_s1.link import Link,transmit,spreading_codes,simulate_channel
from plm_s1.packet import to_packet
from plm_s1.receiver import Session
from plm_s1.evaluation import protocol
...
class IntegrationTests(unittest.TestCase):
    ...
    def test_explicit_spreading_changes_discrete_spectrum(self):
        ...
        for mode in ("repeat","spread"):
            ...
            payload=y[link.payload_start:link.tail_start]
    ...
    def test_revision_target_clause_roundtrip(self):
        ...
        y,m=simulate_channel(tx,link,seed=12,delay_chips=4,phase_rad=1.2,cfo_hz=.1)
...
def r1_case(index):
    def test(self):
        ...
        link=Link()
        tx,norm=transmit(encode(frames,book),book,link)
        y,m=simulate_channel(tx,link,seed=713,delay_chips=-7,phase_rad=-2.0,cfo_hz=.09,keep_fraction=.25,noise_std=.25)
        packet=to_packet(y,m,book,link,norm,source_digest=digest(frames))
        vocab={}
        for old in cases:
            for f in old["frames"]:
