...
WORK=Path(__file__).resolve().parent;ROOT=WORK.parent/'outputs/PLM-L1-SS-doc-v0.3';sys.path.insert(0,str(ROOT))
from evaluation.integrity import write,read,sha,verify

verify();assert read(ROOT/'verification/DOCUMENT_QA.json')['passed']
archive=ROOT.with_name(ROOT.name+'.zip');extracted=WORK/'ssdoc03-zip-extracted'
...
write(ROOT/'RELEASE_MANIFEST.json',{'files':{p.relative_to(ROOT).as_posix():sha(p) for p in files},'eligible_for_inference':False,'frozen_source_digest':verify()})
...
write(WORK/'ssdoc03-package.json',r);print(json.dumps(r,indent=2),flush=True)
