...
class PositiveLexicalBaseline:
    ...
    def analyze(self, texts):
        ...
        for domain in domains:
            ...
            positive_mass = sum(value for value in (scores[row] for row in rows) if value > 0)
            if top_score <= 0:
                selection_confidence = 1.0
            elif tied:
                selection_confidence = 1.0 - (top_score / positive_mass if positive_mass else 0.0)
            else:
                selection_confidence = top_score / positive_mass if positive_mass else 0.0
            selections[domain] = {
                "domain": domain,
                "selected": selected,
        ...
        return {
            "version": "positive-lexical-baseline v0.3",
            "inputs": texts,
