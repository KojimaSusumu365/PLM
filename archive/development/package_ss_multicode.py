"""Seal the multi-code release; extract and verify the actual archive."""
import json
import os
import subprocess
import sys
...
assert repeat['all_equal'] and len(repeat['files']) == 65
...
with tempfile.TemporaryDirectory(prefix='mczip-') as folder:
    ...
    verified.update({'archive': record, 'archive_actually_extracted': True, 'crc_verified': True,
                     'full_evaluations_before_packaging': 2, 'repeat_equal_files': 65,
