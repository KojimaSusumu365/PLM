...
from plm_s1.link import Link,transmit,simulate_channel,spreading_codes
from plm_s1.packet import to_packet
from plm_s1.receiver import Session
...
def main():
    ...
    book=PhaseCodebook(2048,"s1-v01-demo")
    link=Link()
    frames=make_frames(4)
    values=encode(frames,book)
    frame,norm=transmit(values,book,link)
    truth={"seed":61001,"delay_chips":-5,"phase_rad":1.8,"cfo_hz":.12,"keep_fraction":.25,"noise_std":.25,"jitter_std":.03}
    ...
    for mode in ("direct_sparse","repeat","spread"):
        ...
        payload=tx[cfg.payload_start:cfg.tail_start]
        ...
        controls.append({"mode":mode,"frame_chips":len(tx),"duration_seconds":len(tx)/cfg.chip_rate_hz,
                         "total_energy":float(np.vdot(tx,tx).real),"pilot_energy":2*cfg.pilot_length*n["pilot_amplitude"]**2,
