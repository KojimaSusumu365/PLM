...
root=Path(__file__).resolve().parents[1]/'outputs/PLM-L1-v0.11';sys.path.insert(0,str(root))
from evaluation.integrity import sha,verify_freeze
for name in ('REPORT.md','verification/strict/VERIFICATION.json','verification/v010/VERIFICATION.json','verification/REPEATABILITY.json','verification/PRESERVATION_CHECK.json','verification/BENCHMARK.json'):
    assert (root/name).is_file(),name
freeze=verify_freeze();result=json.loads((root/'results/EVALUATION.json').read_text(encoding='utf-8'));assert result['all_checks_passed']
...
with zipfile.ZipFile(archive,'x',compression=zipfile.ZIP_DEFLATED,compresslevel=6) as z:
    for p in sorted(root.rglob('*')):
        if p.is_file() and '__pycache__' not in p.parts:z.write(p,root.name+'/'+p.relative_to(root).as_posix(),compress_type=zipfile.ZIP_STORED if p.suffix=='.zip' else zipfile.ZIP_DEFLATED)
