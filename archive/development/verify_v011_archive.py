...
outputs=Path(__file__).resolve().parents[1]/'outputs';name='PLM-L1-v0.11';archive=outputs/(name+'.zip')
...
with tempfile.TemporaryDirectory(prefix='l11check-') as folder:
    ...
    with zipfile.ZipFile(archive) as z:
        ...
        for info in z.infolist():
            ...
            assert str(dest).casefold() not in seen;seen.add(str(dest).casefold())
        z.extractall(base)
    ...
    out=base/'verified';log_path=outputs/(name+'-ARCHIVE-VERIFICATION.log')
    with log_path.open('x',encoding='utf-8') as log:
        r=subprocess.run([sys.executable,'-B','verify_release.py','--mode','functional','--out',str(out)],cwd=base/name,env=env,stdout=log,stderr=subprocess.STDOUT,timeout=1200)
    ...
    report.update({'archive':record,'archive_actually_extracted':True,'crc_verified':True,'platform_scope':'Same Windows host, fresh temporary directories; not Linux.',
                   'v010_own_tests_run_before_packaging':47,'older_nested_tests_repeated_this_turn':False})
