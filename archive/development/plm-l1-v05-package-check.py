...
SOURCE = OUTPUTS / "PLM-L1-v0.5"
ARCHIVE = OUTPUTS / "PLM-L1-v0.5.zip"
EXTRACT = WORK / "plm-l1-v05-package-extracted"
VERIFY = WORK / "plm-l1-v05-package-verification"
...
execution = subprocess.run([sys.executable, "-B", "verify_release.py", "--out", str(VERIFY)], cwd=extracted, env=env, capture_output=True, text=True, encoding="utf-8", timeout=360)
(WORK / "plm-l1-v05-package-verification.log").write_text(execution.stdout + execution.stderr, encoding="utf-8")
...
try:
    commands = [
        ["-m", "plm_l1_v05", "read", "--model", "results/model", "--text", "花子を太郎が助けなかった。", "--out", packet.name],
        ["-m", "plm_l1_v05", "generate", "--model", "results/model", "--packet", packet.name, "--goal", "subject"],
        ["-m", "plm_l1_v05", "encode", "--model", "results/model", "--meaning", "examples/MEANING.json", "--out", direct.name],
        ["-m", "plm_l1_v05", "generate", "--model", "results/model", "--packet", direct.name, "--goal", "object"],
...
record.update(archive=str(ARCHIVE), archive_sha256=archive_hash, archive_bytes=ARCHIVE.stat().st_size,
              ...
              heldout_read_generate_cli_passed=True, direct_encode_heldout_generate_cli_passed=True, root_json_source_guard_passed=True)
with (OUTPUTS / "PLM-L1-v0.5-VERIFICATION.json").open("x", encoding="utf-8") as stream:
