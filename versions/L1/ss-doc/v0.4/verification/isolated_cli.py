...
try:runpy.run_module('ss_revision',run_name='__main__')
except SystemExit as e:exit_code=e.code or 0
loaded={}
for name,module in sorted(sys.modules.items()):
    if any(name==p or name.startswith(p+'.') for p in ('ss_revision','ss_retention','ss_partial','ss_document','plm_l1_v09')):
...
print(json.dumps({'verified_paths':loaded,'reader_learning_evaluator_absent':True}),file=sys.stderr)
