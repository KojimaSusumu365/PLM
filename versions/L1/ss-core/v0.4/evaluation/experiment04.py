...
def main_trials(out,indices=None):
    ...
    return {'requests':len(rows),'correct':sum(r['correct'] for r in rows),'wrong_text':sum(r['wrong_text'] for r in rows),
            ...
            'reader_ticks_once_per_document':sum(r['ticks'] for p in (out/'traces').glob('read-*.json') for r in read(p)),
...
def update_trials(out,indices=None):
...
def partial_trials(out,indices=None):
    ...
    for i,state in enumerate(('unobserved','unreadable','ambiguous','conflict')):
        if indices is not None and i not in indices:continue
    ...
    return {'requests':len(rows),'held_before':sum(r['held_result']['status']=='held' for r in rows),'correct_after':sum(r['checks']['correct'] for r in rows),
            'states_retained':sum(r['retained_before'] for r in rows),'non_targets_equal':sum(r['non_targets_equal'] for r in rows)}

def fault_trials(out,indices=None):
    case=read(ROOT/'data/WORKING04_CORPUS.json')['regression'][0];packet,_=source(case);rows=[]
    for i,kind in enumerate(read(ROOT/'PROTOCOL.json')['faults']):
        if indices is not None and i not in indices:continue
    ...
    if indices is None or 9 in indices:
...
def run(out,suite,indices=None):
    ...
    for name,fn in functions.items():
        if suite in ('all',name):summary[name]=fn(out,indices)
    ...
    verify();print(json.dumps(summary,ensure_ascii=False),flush=True);return summary
