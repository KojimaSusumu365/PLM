...
def inputs(model, cases, split):
    ...
    for i,case in enumerate(cases):
        ...
        for kind in (('teacher','query') if case['kind']=='focal' else ('teacher',)):
            ...
            assert reception['status']=='received', (case['id'],kind,reception)
            packets[(case['id'],kind)] = reception
            actual = model.recover(reception['packet'])['observation']
            records.append({'case_id':case['id'],'kind':kind,'message_id':mid,'channel_seed':seed,
                            ...
                            'received_observation_equal':actual==(observation if kind=='teacher' else case['query']),
...
def train(model,cases,packets,seed,mode,out):
    ...
    receipts = []
    for case in cases:
        packet = packets[(case['id'],'teacher')]['packet']
        if mode=='batch':
