...
FORMAT="PLM-S1-chip-observation-packet-v2"
...
def to_packet(samples,mask,book,link,normalization,*,source_digest):
    ...
    y[~m]=0
    packet={"format":FORMAT,"producer_version":"PLM-S1 v0.2","codebook":book.spec,"codebook_fingerprint":book.fingerprint,
...
def from_packet(packet,*,expected_book=None,expected_link=None):
    ...
    require(packet["format"]==FORMAT and packet["producer_version"]=="PLM-S1 v0.2","Unsupported packet version")
    ...
    expected_amplitude=math.sqrt(norm["total_tx_energy"]/(link.payload_length+256))
