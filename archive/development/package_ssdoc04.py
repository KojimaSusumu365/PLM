...
WORK=Path(__file__).resolve().parent;ROOT=WORK.parent/'outputs/PLM-L1-SS-doc-v0.4';sys.path.insert(0,str(ROOT))
from evaluation.integrity import write,read,sha,verify

verify();assert read(ROOT/'verification/DOCUMENT_QA.json')['passed']
archive=ROOT.with_name(ROOT.name+'.zip');extracted=WORK/'ssdoc04-zip-extracted'
...
write(WORK/'ssdoc04-package.json',r);print(json.dumps(r,indent=2),flush=True)
