...
SOURCE = OUTPUTS / "PLM-L1-v0.4"
ARCHIVE = OUTPUTS / "PLM-L1-v0.4.zip"
EXTRACT = WORK / "plm-l1-v04-package-extracted"
VERIFY = WORK / "plm-l1-v04-package-verification"
...
execution = subprocess.run([sys.executable, "-B", "verify_release.py", "--out", str(VERIFY)], cwd=extracted, env=env, capture_output=True, text=True, encoding="utf-8", timeout=300)
(WORK / "plm-l1-v04-package-verification.log").write_text(execution.stdout + execution.stderr, encoding="utf-8")
...
try:
    commands = [
        ["-m", "plm_l1_v04", "read", "--model", "results/model", "--text", "花子を太郎が助けなかった。", "--out", packet.name],
        ["-m", "plm_l1_v04", "generate", "--model", "results/model", "--packet", packet.name, "--goal", "subject"],
        ["-m", "plm_l1_v04", "encode", "--model", "results/model", "--meaning", "examples/MEANING.json", "--out", direct.name],
        ["-m", "plm_l1_v04", "generate", "--model", "results/model", "--packet", direct.name, "--goal", "object"],
...
with (OUTPUTS / "PLM-L1-v0.4-VERIFICATION.json").open("x", encoding="utf-8") as stream:
