...
def inventory():
    files = {p.relative_to(ROOT).as_posix(): sha(p) for folder in ('ss_multicode', 'ss_active', 'evaluation', 'data', 'tests')
             for p in sorted((ROOT/folder).rglob('*')) if p.is_file() and '__pycache__' not in p.parts and p.name != 'FREEZE.json'}
    return files | {n: sha(ROOT/n) for n in ('evaluate.py', 'verify_release.py', 'requirements.txt')}
