...
原本文書のうち、旧 `outputs/` 配置や大容量ファイルを指す相対リンクは、Gitの閲覧用ツリーでは開けない場合があります。原本を改変してリンクを書き換えることはせず、[版別索引](VERSION_INDEX.md) と各版の `_ARCHIVE_RETRIEVAL.md` を新しい入口として追加しています。旧配置で参照したい場合は `--scope outputs` で復元してください。
