...
SOURCE = OUTPUTS / "PLM-L1-v0.7"
ARCHIVE = OUTPUTS / "PLM-L1-v0.7.zip"
EXTRACT = WORK / "plm-l1-v07-package-extracted"
VERIFY = WORK / "plm-l1-v07-package-verification"
...
(WORK / "plm-l1-v07-package-verification.log").write_text(execution.stdout + execution.stderr, encoding="utf-8")
...
try:
    commands = [
        ["-m", "plm_l1_v07", "read", "--model", "results/model", "--text", "太郎が花子を助けた。花子が太郎を助けなかった。", "--out", packet.name],
        ["-m", "plm_l1_v07", "generate", "--model", "results/model", "--packet", packet.name, "--goals", "object", "subject"],
        ["-m", "plm_l1_v07", "encode", "--model", "results/model", "--meaning", "examples/MEANING.json", "--out", direct.name],
        ["-m", "plm_l1_v07", "generate", "--model", "results/model", "--packet", direct.name, "--goals", "subject", "object"],
    ...
    for i, command in enumerate(commands):
        ...
        if i == 1:
            assert json.loads(executed.stdout)["text"] == "花子を太郎が助けた。花子が太郎を助けなかった。"
        elif i == 3:
            assert json.loads(executed.stdout)["text"] == "太郎が花子を助けた。太郎を花子が助けなかった。"
finally:
...
record.update(archive=str(ARCHIVE), archive_sha256=archive_hash, archive_bytes=ARCHIVE.stat().st_size,
              ...
              two_event_read_generate_cli_passed=True, two_event_direct_encode_generate_cli_passed=True, source_freeze_checked_after_cli=True)
with (OUTPUTS / "PLM-L1-v0.7-VERIFICATION.json").open("x", encoding="utf-8") as stream:
