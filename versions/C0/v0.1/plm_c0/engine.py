...
class PLMC0Engine:
    """PLM-C0 v0.1: inspectable ternary Concept consensus proof-of-concept."""
    ...
    def analyze(self, texts):
        ...
        for d in domains:
            ...
            ranking[d] = [{"concept":s.concept,"label_ja":s.label_ja,"score":round(s.score,4),"direct_support":round(s.direct_support,4),"propagated_support":round(s.propagated_support,4),"contradiction":round(s.contradiction,4),"context_boost":round(s.context_boost,4),"depth":s.depth} for s in rows[:8]]
        return {"version":"PLM-C0 v0.1","inputs":texts,"selections":{d:self._select_domain(d,scores) for d in domains},"ranking":ranking,"evidence":[asdict(e) for e in evidence]}
